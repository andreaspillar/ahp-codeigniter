  ...
  <div class="card">
    <div class="card card-plain">
      <div class="content table-responsive table-full-width">
        <form id="form" action="<?php // echo site_url('HR/updNiQa'); ?>" method="post">
          <table class="table table-hover table-striped">
            ...
            <tr>
              <td colspan="2"><a class="btn btn-warning btn-fill" href="<?php echo site_url('PU/karyawanku/'); ?>" onmouseover="demo.showBack('top','center');" name="button"><i class="pe-7s-back"></i>&nbsp&nbsp Kembali</a>
                <button class="btn btn-fill btn-info butonSB" id="btn" type="submit" >Nilai Karyawan</button></td>
              </tr>
            <?php } ?>
          </table>
        </form>
        <script type="text/javascript">
          $('#btn').click(function(event) {
           ...
           $.ajax({
             type: "POST",
             url: "<?php  echo site_url('HR/updNiQa'); ?>",
             data: form,

             success: function(data){
                 <?php foreach ($nilK as $a): ?>
                  $('#clk').load("<?php echo site_url('HR/dataKDJ/'.$a->jabatan.'/'.$dI->divisi); ?>");
                 <?php endforeach; ?>
...
<?php elseif($this->session->userdata('logged')['level']==='-1'): ?>
  <div class="card">
    <div class="card card-plain">
      <div class="content table-responsive table-full-width">
        <form id="form" action="<?php //echo site_url('PU/updNiQa'); ?>" method="post">
          <table class="table table-hover table-striped">
            <?php foreach ($nilK as $a){
              $ra = $a->id_karyawan;
              ?>
        ...
        <script type="text/javascript">
          $('#btn').click(function(event) {
           ...
           $.ajax({
             type: "POST",
             url: "<?php  echo site_url('PU/updNiQa'); ?>",
             data: form,
